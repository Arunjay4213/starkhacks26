"""
Brain module — calls Claude with a camera frame and transcript,
returns a validated BrainResponse.
"""
...
def _get_system_prompt() -> str:
    """Load system_prompt.txt, hot-reloading on file change."""
...
def plan_grasp(frame_b64: str, transcript: str) -> Optional[BrainResponse]:
    """Call Claude with a frame and transcript. Returns BrainResponse or None."""
    model = os.environ.get("MONDAY_CLAUDE_MODEL", DEFAULT_MODEL)
    client = _get_client()
...
def validate_response(data: dict) -> Optional[BrainResponse]:
    """Validate response JSON against the brain contract."""
